...
class LoanRequest(models.Model):
    ...
    employee_id = fields.Many2one('hr.employee', string='Employee',
                                  domain=[('join_date', '<=', fields.Date.today() - timedelta(days=90))])
    ...
    installment_ids = fields.One2many('loan.installment',
                                      'loan_id', string='Installments')

    is_loan_done = fields.Boolean(string='the loan Fully Done',computed="_computed_amount_loan",
    ...
    next_payment_amount = fields.Float(string="Next Payment amount",
                                       compute="_compute_next_payment_amount")

    salary = fields.Float(string='The Salary Of Employee')

    salary_deduction = fields.Float (string='The salary after deduction',
